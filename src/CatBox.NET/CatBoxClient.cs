...
public class CatBoxClient : ICatBoxClient
{
    ...
    public async Task<string?> CreateAlbum( CreateAlbumRequest createAlbumRequest, CancellationToken ct = default )
    {
        ...
        if( string.IsNullOrWhiteSpace( createAlbumRequest.Description ) )
            throw new ArgumentNullException( nameof( createAlbumRequest.Description ), "Album description cannot be null, empty, or whitespace" );

        if( string.IsNullOrWhiteSpace( createAlbumRequest.Title ) )
            throw new ArgumentNullException( nameof( createAlbumRequest.Title ), "Album title cannot be null, empty, or whitespace" );
        ...
        var links = createAlbumRequest.Files.Select( link =>
        {
            ...
            {
                return new Uri( link ).PathAndQuery.Substring(1);
            }
            return link;
        } );
    ...
    public async Task<string?> EditAlbum( EditAlbumRequest? editAlbumRequest, CancellationToken ct = default )
    {
        ...
        if( string.IsNullOrWhiteSpace( editAlbumRequest.UserHash ) )
            throw new ArgumentNullException( nameof( editAlbumRequest.UserHash ), "UserHash cannot be null, empty, or whitespace when attempting to modify an album" );

        if( string.IsNullOrWhiteSpace( editAlbumRequest.Description ) )
            throw new ArgumentNullException( nameof( editAlbumRequest.Description ), "Album description cannot be null, empty, or whitespace" );

        if( string.IsNullOrWhiteSpace( editAlbumRequest.Title ) )
            throw new ArgumentNullException( nameof( editAlbumRequest.Title ), "Album title cannot be null, empty, or whitespace" );

        if( string.IsNullOrWhiteSpace( editAlbumRequest.AlbumId ) )
            throw new ArgumentNullException( nameof( editAlbumRequest.AlbumId ), "AlbumId (Short) cannot be null, empty, or whitespace" );
    ...
    public async Task<string?> ModifyAlbum( AlbumRequest? albumRequest, CancellationToken ct = default )
    {
        ...
        if( string.IsNullOrWhiteSpace( albumRequest.UserHash ) )
            throw new ArgumentNullException( nameof( albumRequest.UserHash ), "UserHash cannot be null, empty, or whitespace when attempting to modify an album" );
